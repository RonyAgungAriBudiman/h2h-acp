<?php
function cekSo($accessToken, $session, $host, $dari, $sampai, $sqlLib)
{
    // Header
    $header = array(
...
function getSo($accessToken, $session, $host, $dari, $sampai, $sqlLib)
{
    ...
    foreach ($row_result_api->d as $row_api) {
        ...
        $noso = $row_hd->d->number;
        $transDate = $row_hd->d->transDate;
        $customer = $row_hd->d->customer->name;
        $alamat = $row_hd->d->toAddress;
        $subtotal = $row_hd->d->subTotal;
        $tax2amount = $row_hd->d->tax2Amount;
        $totalamount = $row_hd->d->totalAmount;


        $tanggal_obj2 = DateTime::createFromFormat('d/m/Y', $transDate);
        $tanggalso = $tanggal_obj2->format('Y-m-d');

        /*
        cek apakah sudah dibuat draf dokumen bc untuk kirim ke ceisa,
        jika sudah maka tidak bisa di hapus, 
        jika belum maka bisa di hapus
        */
        $sql_aju = "SELECT NoPO FROM BC_HEADER WHERE NoPO ='" . $noso . "' ";
        $data_aju = $sqlLib->select($sql_aju);
        if (count($data_aju) < 1) //jika po/so belum di buat pengajuan maka bisa di replace
        {
            //delete so
            $sql_del = "DELETE FROM ac_so WHERE NoSO = '" . $noso . "' ";
            $run1 = $sqlLib->delete($sql_del);

            $sql_del2 = "DELETE FROM ac_so_detail WHERE NoSO = '" . $noso . "' ";
            $run2 = $sqlLib->delete($sql_del2);


            if ($noso != "") {
                $sql_save = "INSERT INTO ac_so
                                        (NoSO, TanggalSo, Customer, Alamat, Subtotal, Tax2Amount, TotalAmount, recuser, recdate)
                                        VALUES('" . $noso . "' ,'" . $tanggalso . "','" . $customer . "','" . $alamat . "','" . $subtotal . "'  ,'" . $tax2amount . "','" . $totalamount . "',
                                        '" . $_SESSION['userid'] . "','" . date("Y-m-d H:i:s") . "') ";
                $run = $sqlLib->insert($sql_save);
                if ($run == "1") {
                    
                    foreach ($row_hd->d->detailItem as $row) // detail so
                    {

                        $kdbarang = $row->item->no;
                        ...
                        $totalharga = $row->totalPrice;

                        $sql_save_dt = "INSERT INTO ac_so_detail 
                                        (NoSO,KdBarang,NamaBarang,Harga,Qty,ItemDiscPercent, ItemCashDiscount, Satuan, TotalHarga, recUser, recDate)
                                        VALUES('" . $noso . "','" . $kdbarang . "','" . $namabarang . "','" . $harga . "','" . $qty . "','" . $itemDiscPercent . "','" . $itemCashDiscount . "',
                                                '" . $satuan . "','" . $totalharga . "', '" . $_SESSION["userid"] . "','" . date("Y-m-d H:i:s") . "')";
                    ...
                    //cek detail
                    $sql_dtl = "SELECT COUNT(NoSO) as jmldata FROM ac_so_detail WHERE NoSO = '" . $noso . "' ";
                    $data_dtl = $sqlLib->select($sql_dtl);
                    if ($data_dtl[0]['jmldata'] < 1) {
                        $sql_del3 = "DELETE FROM ac_so WHERE NoSO = '" . $noso . "' ";
...
function cekPo($accessToken, $session, $host, $dari, $sampai, $sqlLib)
{
    // Header
    $header = array(
        "Content-Type: application/json",
    ...
    // URL 
    $url = $host . "/accurate/api/purchase-order/list.do?" . http_build_query($content);
...
function getPo($accessToken, $session, $host, $dari, $sampai, $sqlLib)
{
    // Header
    $header = array(
        "Content-Type: application/json",
    ...
    foreach ($row_result_api->d as $row_api) {
        ...
        // URL
        $url = $host . "/accurate/api/purchase-order/detail.do?" . http_build_query($content);
        ...
        $tanggal_obj2 = DateTime::createFromFormat('d/m/Y', $transDate);
        $tanggalpo = $tanggal_obj2->format('Y-m-d');
        ...
        jika belum maka bisa di hapus
        */
        $sql_aju = "SELECT NoPO FROM BC_HEADER WHERE NoPO ='" . $nopo . "' ";
        $data_aju = $sqlLib->select($sql_aju);
        if (count($data_aju) < 1) //jika po belum di buat pengajuan maka bisa di replace
        {
            //delete po
            $sql_del = "DELETE FROM ac_po WHERE NoPO = '" . $nopo . "' ";
            $run1 = $sqlLib->delete($sql_del);

            $sql_del2 = "DELETE FROM ac_po_detail WHERE NoPO = '" . $nopo . "' ";
            $run2 = $sqlLib->delete($sql_del2);


            if ($nopo != "") {
                $sql_save = "INSERT INTO ac_po
                                        (NoPO, TanggalPo, Vendor, Alamat, Subtotal, Tax2Amount, TotalAmount, recuser, recdate)
                                        VALUES('" . $nopo . "' ,'" . $tanggalpo . "','" . $vendor . "','" . $alamat . "','" . $subtotal . "'  ,'" . $tax2amount . "','" . $totalamount . "',
                                        '" . $_SESSION['userid'] . "','" . date("Y-m-d H:i:s") . "') ";
                $run = $sqlLib->insert($sql_save);
                if ($run == "1") {

                    foreach ($row_hd->d->detailItem as $row) // detail po
                    {
                        ...
                        $sql_save_dt = "INSERT INTO ac_po_detail 
                                        (NoPO,KdBarang,NamaBarang,Harga,Qty,ItemDiscPercent,ItemCost,Satuan, TotalHarga, recUser, recDate)
                                        VALUES('" . $nopo . "','" . $kdbarang . "','" . $namabarang . "','" . $harga . "','" . $qty . "','" . $itemDiscPercent . "','" . $itemCost . "','" . $satuan . "',
                                                '" . $totalharga . "', '" . $_SESSION["userid"] . "','" . date("Y-m-d H:i:s") . "')";
                        $run_dt = $sqlLib->insert($sql_save_dt);
                        //echo $sql_save_dt;
                    }

                    //cek detail
                    $sql_dtl = "SELECT COUNT(NoPO) as jmldata FROM ac_po_detail WHERE NoPO = '" . $nopo . "' ";
                    $data_dtl = $sqlLib->select($sql_dtl);
                    if ($data_dtl[0]['jmldata'] < 1) {
                        $sql_del3 = "DELETE FROM ac_po WHERE NoPO = '" . $nopo . "' ";
                        $run3 = $sqlLib->delete($sql_del3);
                    }
