...
function getStockGudang_($accessToken, $session, $host, $dari, $sampai, $sqlLib)
{
...
function getStockProduksi($accessToken, $session, $host, $dari, $sampai, $sqlLib)
{
    ...
    //delete Stock
    //$sql_del = "DELETE FROM ac_stock WHERE tanggal = '" . $tanggal . "' AND awal ='0' AND lokasiGudang='Produksi'";
    //$run_del = $sqlLib->delete($sql_del);
...
function getStockMesin($accessToken, $session, $host, $dari, $sampai, $sqlLib)
{
    ...
    foreach ($row_result_api->d as $row_api) {
        $transDate = $row_api->transDate;
        $quantityDisposed = $row_api->quantityDisposed; //qty pindah 
        $quantity = $row_api->quantity;
        $draft = $row_api->draft;
        $disposed = $row_api->disposed;
        $itemNo = $row_api->number;
        $itemName = $row_api->description;
        $itemCategory = "Mesin";
        $unit1Name = "Unit";
        if ($disposed == "1") {
            $masuk = 0;
        ...
        if ($dari == $transDate and $draft != "1") {
            ...
            if (count($data_cek) > 0) {
                ...
                //cek stock sebelumnya
                $sql_kemarin = "SELECT TOP 1 COALESCE(akhir,NULL,0) as stockkemarin FROM ac_stock WHERE itemNo = '" . $itemNo . "' AND lokasiGudang ='Barang Modal' Order By tanggal desc ";
                $data_kemarin = $sqlLib->select($sql_kemarin);
                if ($data_kemarin[0]['stockkemarin'] == "") {
                    $qtyawal = 0;
                } else {
                ...
                $sql_save = "INSERT INTO ac_stock (tanggal, itemNo, itemName, unit1Name, itemCategory, awal, masuk, keluar, akhir, adjustment, so, lokasiGudang, itemId, recuser, recdate)
                                    VALUES ('" . $tanggal . "','" . $itemNo . "','" . $itemName . "','" . $unit1Name . "','" . $itemCategory . "','" . $qtyawal . "','" . $masuk . "','" . $keluar . "','" . $qtyakhir . "','0' ,'0' ,
                                            'Barang Modal','" . $itemId . "', '" . $_SESSION['userid'] . "','" . date("Y-m-d H:i:s") . "') ";
                $run_save = $sqlLib->insert($sql_save);
            }
        }
        //delete Stock
        $sql_del3 = "DELETE FROM ac_stock WHERE tanggal = '" . $tanggal . "' AND itemNo = '" . $itemNo . "' AND lokasiGudang ='Barang Modal'
                             AND awal = '0' AND masuk = '0' AND keluar = '0' AND akhir = '0'  AND adjustment = '0'  AND so = '0'  ";
        $run_del3 = $sqlLib->delete($sql_del3);
