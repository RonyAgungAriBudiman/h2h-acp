...
if ($_FILES["file"]["name"] != "") {
	...
	if ($isi_header > 0) {
		...
		if ($isi_entitas >= "3") {
						...
						if ($isi_barang > 0) {
							...
							if ($isi_barang_tarif >= 0) {
								...
								if ($isi_pungutan >= 0) {
									...
									if ($isi_kontainer > 0) {
										$isi_bankdevisa = 0;
										for ($i = 2; $i <= $jmlrow_bank_devisa; $i++) {
											$seri = $data->sheets[17]['cells'][$i][2];
											$kode = $data->sheets[17]['cells'][$i][3];
											$nama = $data->sheets[17]['cells'][$i][4];
											$sql_bd = "INSERT INTO BC_BANK_DEVISA (
												NomorAju, Seri, Kode, Nama,  RecUser) VALUES (
												'" . $nomoraju . "', '" . $seri . "', '" . $kode . "', '" . $nama . "','" . $_SESSION["nama"] . "')";
										...
										if ($isi_bankdevisa > 0) {
											...
											$sql_kon = "DELETE FROM BC_KONTAINER WHERE NomorAju = '" . $nomoraju . "'";
											$data_kon = $sqlLib->delete($sql_kon);
			...
			$alert = '1';
			$note = "Proses simpan entitas gagal!!" . $sql_entitas;
		}
	} else {
		$alert = '1';
