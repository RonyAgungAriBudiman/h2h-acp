...
if ($_FILES["file"]["name"] != "") {
	...
	for ($i = 2; $i <= $jmlrow_header; $i++) {
		...
		$sql_header = "INSERT INTO BC_HEADER (
				...
				'" . $barangtidakberwujud . "','" . $kodejenispengeluaran . "','" . $_POST["noso"] . "', '" . $urut . "','" . $_SESSION["nama"] . "')";
						...
						if ($isi_barang > 0) {
							...
							if ($isi_barang_tarif >= 0) {
								...
								if ($isi_pungutan >= 0) {
									//save bahan baku
									$isi_bahanbaku = 0;
									for ($i = 2; $i <= $jmlrow_bahan_baku; $i++) {
										//$nomoraju = $data->sheets[12]['cells'][$i][1];
										$seribarang = $data->sheets[12]['cells'][$i][2];
										$seribahanbaku = $data->sheets[12]['cells'][$i][3];
										$kodeasalbahanbaku = $data->sheets[12]['cells'][$i][4];
										$hs = $data->sheets[12]['cells'][$i][5];
										$kodebarang = $data->sheets[12]['cells'][$i][6];
										$uraian = $data->sheets[12]['cells'][$i][7];
										$merek = $data->sheets[12]['cells'][$i][8];
										$tipe = $data->sheets[12]['cells'][$i][9];
										$ukuran = $data->sheets[12]['cells'][$i][10];
										$spesifikasilain = $data->sheets[12]['cells'][$i][11];
										$kodesatuan = $data->sheets[12]['cells'][$i][12];
										$jumlahsatuan = $data->sheets[12]['cells'][$i][13];
										$kodekemasan = $data->sheets[12]['cells'][$i][14];
										$jumlahkemasan = $data->sheets[12]['cells'][$i][15];
										$kodedokumenasal = $data->sheets[12]['cells'][$i][16];
										$kodekantorasal = $data->sheets[12]['cells'][$i][17];
										$nomordaftarasal = $data->sheets[12]['cells'][$i][18];
										$tanggaldaftarasal = $data->sheets[12]['cells'][$i][19];
										$nomorajuasal = $data->sheets[12]['cells'][$i][20];
										$seribarangasal = $data->sheets[12]['cells'][$i][21];
										$netto = $data->sheets[12]['cells'][$i][22];
										$bruto = $data->sheets[12]['cells'][$i][23];
										$volume = $data->sheets[12]['cells'][$i][24];
										$cif = $data->sheets[12]['cells'][$i][25];
										$cifrupiah = $data->sheets[12]['cells'][$i][26];
										$ndpbm = $data->sheets[12]['cells'][$i][27];
										$hargapenyerahan = $data->sheets[12]['cells'][$i][28];
										$hargaperolehan = $data->sheets[12]['cells'][$i][29];
										$nilaijasa = $data->sheets[12]['cells'][$i][30];
										$seriizin = $data->sheets[12]['cells'][$i][31];
										$valuta = $data->sheets[12]['cells'][$i][32];
										$kodebkc = $data->sheets[12]['cells'][$i][33];
										$kodekomoditibkc = $data->sheets[12]['cells'][$i][34];
										$kodesubkomoditibkc = $data->sheets[12]['cells'][$i][35];
										$flagtis = $data->sheets[12]['cells'][$i][36];
										$isiperkemasan = $data->sheets[12]['cells'][$i][37];
										$jumlahdilekatkan = $data->sheets[12]['cells'][$i][38];
										$jumlahpitacukai = $data->sheets[12]['cells'][$i][39];
										$hjecukai = $data->sheets[12]['cells'][$i][40];
										$tarifcukai = $data->sheets[12]['cells'][$i][41];
										$sql_bahanbaku = "INSERT INTO BC_BAHAN_BAKU (
												NomorAju, SeriBarang, SeriBahanBaku, KodeAsalBahanBaku, Hs, KodeBarang, Uraian, Merek, Tipe, Ukuran, SpesifikasiLain, KodeSatuan, JumlahSatuan, KodeKemasan, 
												JumlahKemasan, KodeDokumenAsal, KodeKantorAsal, NomorDaftarAsal, TanggalDaftarAsal, NomorAjuAsal, SeriBarangAsal, Netto, Bruto, Volume, Cif, CifRupiah, 
												Ndpbm, HargaPenyerahan, HargaPerolehan, NilaiJasa, SeriIzin, Valuta, KodeBkc, KodeKomoditiBkc, KodeSubKomoditiBkc, FlagTis, IsiPerKemasan, JumlahDilekatkan, 
												JumlahPitaCukai, HjeCukai, TarifCukai, RecUser ) VALUES (
												'" . $nomoraju . "', '" . $seribarang . "', '" . $seribahanbaku . "', '" . $kodeasalbahanbaku . "', '" . $hs . "', '" . $kodebarang . "', '" . $uraian . "', '" . $merek . "', '" . $tipe . "', 
												'" . $ukuran . "', '" . $spesifikasilain . "', '" . $kodesatuan . "', '" . $jumlahsatuan . "', '" . $kodekemasan . "', '" . $jumlahkemasan . "', '" . $kodedokumenasal . "', 
												'" . $kodekantorasal . "', '" . $nomordaftarasal . "', '" . $tanggaldaftarasal . "', '" . $nomorajuasal . "', '" . $seribarangasal . "', '" . $netto . "', '" . $bruto . "', 
												'" . $volume . "', '" . $cif . "', '" . $cifrupiah . "', '" . $ndpbm . "', '" . $hargapenyerahan . "', '" . $hargaperolehan . "', '" . $nilaijasa . "', '" . $seriizin . "', '" . $valuta . "', 
												'" . $kodebkc . "', '" . $kodekomoditibkc . "', '" . $kodesubkomoditibkc . "', '" . $flagtis . "', '" . $isiperkemasan . "', '" . $jumlahdilekatkan . "', '" . $jumlahpitacukai . "', 
												'" . $hjecukai . "', '" . $tarifcukai . "','" . $_SESSION["nama"] . "')";
										$save_bahanbaku = $sqlLib->insert($sql_bahanbaku);
										if ($save_bahanbaku == "1") {
											$isi_bahanbaku++;
										}
									}
									if ($isi_bahanbaku > 0) {
										...
										$isi_bahanbaku_tarif = 0;
										for ($i = 2; $i <= $jmlrow_bahan_baku_tarif; $i++) {
											//$nomoraju = $data->sheets[13]['cells'][$i][1];
											$seribarang = $data->sheets[13]['cells'][$i][2];
											$seribahanbaku = $data->sheets[13]['cells'][$i][3];
											$kodeasalbahanbaku = $data->sheets[13]['cells'][$i][4];
											$kodepungutan = $data->sheets[13]['cells'][$i][5];
											$kodetarif = $data->sheets[13]['cells'][$i][6];
											$tarif = $data->sheets[13]['cells'][$i][7];
											$kodefasilitas = $data->sheets[13]['cells'][$i][8];
											$tariffasilitas = $data->sheets[13]['cells'][$i][9];
											$nilaibayar = $data->sheets[13]['cells'][$i][10];
											$nilaifasilitas = $data->sheets[13]['cells'][$i][11];
											$nilaisudahdilunasi = $data->sheets[13]['cells'][$i][12];
											$kodesatuan = $data->sheets[13]['cells'][$i][13];
											$jumlahsatuan = $data->sheets[13]['cells'][$i][14];
											$flagbmtsementara = $data->sheets[13]['cells'][$i][15];
											$kodekomoditicukai = $data->sheets[13]['cells'][$i][16];
											$kodesubkomoditicukai = $data->sheets[13]['cells'][$i][17];
											$flagtis = $data->sheets[13]['cells'][$i][18];
											$flagpelekatan = $data->sheets[13]['cells'][$i][19];
											$kodekemasan = $data->sheets[13]['cells'][$i][20];
											$kodesubkomoditicukai = $data->sheets[13]['cells'][$i][21];
											$flagtis = $data->sheets[13]['cells'][$i][22];
											$flagpelekatan = $data->sheets[13]['cells'][$i][23];
											$kodekemasan = $data->sheets[13]['cells'][$i][24];
											$jumlahkemasan = $data->sheets[13]['cells'][$i][25];
											$sql_bahanbaku_tarif = "INSERT INTO BC_BAHAN_BAKU_TARIF (
													NomorAju, SeriBarang, SeriBahanBaku, KodeAsalBahanBaku, KodePungutan, KodeTarif, Tarif, KodeFasilitas, TarifFasilitas, NilaiBayar, NilaiFasilitas,
													NilaiSudahDilunasi, KodeSatuan, JumlahSatuan, FlagBmtSementara, KodeKomoditiCukai, KodeSubKomoditiCukai, FlagTis, FlagPelekatan, KodeKemasan,
													JumlahKemasan, RecUser) VALUES (
													'" . $nomoraju . "', '" . $seribarang . "', '" . $seribahanbaku . "', '" . $kodeasalbahanbaku . "', '" . $kodepungutan . "', '" . $kodetarif . "', '" . $tarif . "', 
													'" . $kodefasilitas . "', '" . $tariffasilitas . "', '" . $nilaibayar . "', '" . $nilaifasilitas . "', '" . $nilaisudahdilunasi . "', '" . $kodesatuan . "', 
													'" . $jumlahsatuan . "', '" . $flagbmtsementara . "', '" . $kodekomoditicukai . "', '" . $kodesubkomoditicukai . "', '" . $flagtis . "', '" . $flagpelekatan . "', 
													'" . $kodekemasan . "', '" . $jumlahkemasan . "', '" . $_SESSION["nama"] . "')";
											$save_bahanbaku_tarif = $sqlLib->insert($sql_bahanbaku_tarif);
											if ($save_bahanbaku_tarif == "1") {
												$isi_bahanbaku_tarif++;
											}
										}
										if ($isi_bahanbaku_tarif > 0) {
											//save kontainer
											$isi_kontainer = 0;
											...
											if ($isi_kontainer > 0) {
												...
												$sql_bnb = "DELETE FROM BC_BAHAN_BAKU WHERE NomorAju = '" . $nomoraju . "'";
												$data_bnb = $sqlLib->delete($sql_pun);
												$sql_pun = "DELETE FROM BC_PUNGUTAN WHERE NomorAju = '" . $nomoraju . "'";
											...
											$alert = '1';
											$note = "Proses simpan bahan baku tarif gagal!!";
										}
									} else {
										$sql_pun = "DELETE FROM BC_PUNGUTAN WHERE NomorAju = '" . $nomoraju . "'";
										$data_pun = $sqlLib->delete($sql_pun);
										...
										$alert = '1';
										$note = "Proses simpan bahan baku gagal!!";
									}
								} else {
									$sql_bat = "DELETE FROM BC_BARANG_TARIF WHERE NomorAju = '" . $nomoraju . "'";
									$data_bat = $sqlLib->delete($sql_bat);
